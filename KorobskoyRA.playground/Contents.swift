...
var floatA: Float? = nil
var floatB: Float? = 2.5

func checkNil(number: Float?) {
    print(number ?? "Variable can't be unwrapped")
}
...
func convertString(string: String) -> String {
    ...
    var lastChar = Int(String(mutateString[mutateString.index(before: mutateString.endIndex)])) // определяем последний символ
    print(lastChar!)
    if lastChar != nil && lastChar != 9 { // проверяем наличие
        ...
        var preLastChar = Int(String(mutateString[mutateString.index(mutateString.startIndex, offsetBy: mutateString.count - 2)])) // вычисляем предпоследний
